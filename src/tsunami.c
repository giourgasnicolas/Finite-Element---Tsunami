...
void tsunamiAnimate(double dt, int nmax, int sub,  const char *meshFileName, const char *baseResultName)
{
    ...
    do {
        ...
        if (frame0 != frame) {
            ...
            glfwGetCursorPos(window, &mouse, NULL );   
            mouse = 389;  
            const char *basename = "%s-%08d.txt";
            char filename[256];
            ...
            mouse = glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT);
            if(mouse == 1.0){
                glfwGetCursorPos(window, &x, &y);
            }
            glRotatef((GLfloat)x + 110,0.0f,0.0f,1.0f);
            glRotatef((GLfloat)y + 350,0.0f, 1.0f, 0.0f);

            GLUquadricObj *quadratic = gluNewQuadric();         
            gluQuadricNormals(quadratic, GLU_SMOOTH); 
            glColor3f(.25,.89,.05);//rouge vert bleu
